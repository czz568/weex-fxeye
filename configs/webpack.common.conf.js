...
const weexEntry = {
  ...
  'market': helper.root('market.js'),
  'inner': helper.root('inner.js')
}
...
const getEntryFile = () => {
  ...
  const marketFilePath = 'market.js'
  const marketFile = path.join(vueWebTemp, marketFilePath)
  fs.outputFileSync(marketFile,getEntryFileContent(helper.root(marketFilePath),routerFile));
  const innerFilePath = 'inner.js'
  const innerFile = path.join(vueWebTemp, innerFilePath)
  fs.outputFileSync(innerFile,getEntryFileContent(helper.root(innerFilePath),routerFile));
  return {
    index: entryFile,
    market: marketFile,
    inner: innerFile
